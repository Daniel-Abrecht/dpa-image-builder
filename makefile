...
emulate: bin/$(IMAGE_NAME)
	qemu-system-aarch64 -M virt -cpu cortex-a53 -m 3G -kernel kernel/linux/debian/tmp/boot/vmlinuz-* -append "root=/dev/vda3" -drive if=none,file=bin/$(IMAGE_NAME),format=raw,id=hd -device virtio-blk-device,drive=hd -nographic
